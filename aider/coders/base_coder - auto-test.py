...
class Coder:
    ...
    def handle_shell_commands(self, commands_str, group):
        ...
        for cmd in commands:
            ...
            try:
                ...
                for part in cmd_parts:
                    if part in ("|", "&&", ";"): continue 
                    if "=" in part or part.startswith("-"): continue 
                    base_cmd = Path(part).name
                    if base_cmd in wrappers: continue 
        ...
        log_file_path = Path(self.abs_root_path(log_file_name))
        prev_log_path = Path(self.abs_root_path(prev_log_name))
        ...
        if token_count <= ERROR_BUDGET_TOKENS:
            ...
            if is_read_cmd:
                ...
                processed_output = (
                    ...
                    f"--- SYSTEM INSTRUCTION ---\n"
                    f"Command succeeded (exit code 0). Logs hidden to save tokens."
                )
            ...
            if len(lines) <= HEAD_LINES + TAIL_LINES:
                ...
                last_end = 0
                for start, end in kept_zones:
                    if start > last_end:
                        omitted = start - last_end
                        middle_extracted.append(f"\n... [{omitted} lines omitted] ...\n")
                        
                    middle_extracted.extend(middle_lines[start:end])
                    last_end = end
                    
                if not merged_zones:
                    middle_extracted.append("\n... [MASSIVE LOG TRUNCATED - No explicit error keywords found in middle] ...\n")
                elif last_end < len(middle_lines):
                    omitted = len(middle_lines) - last_end
                    middle_extracted.append(f"\n... [{omitted} lines omitted] ...\n")

                processed_output = (
                    "\n".join(head_block) + "\n" +
                    "\n".join(middle_extracted) + "\n" +
                    "\n".join(tail_block) + "\n\n" +
                    f"--- SYSTEM INSTRUCTION ---\n" +
                ...
                self.io.tool_output(sys_msg)
                processed_output += f"\n{sys_msg}"

        accumulated_output = processed_output
